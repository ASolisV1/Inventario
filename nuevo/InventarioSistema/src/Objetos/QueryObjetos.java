...
public class QueryObjetos extends Conexion {
    public boolean guardar(ClsObjetos clase) {
...
public boolean modificar(ClsObjetos clase) {
        PreparedStatement ps=null;
        Connection con = getConexion();
        String sql="UPDATE tbl_personas SET nombres=? WHERE id=?";
        try {            
            ps = con.prepareStatement(sql);
            ps.setString(1,clase.getNombres());
            ps.execute();
            return true;
        } catch(SQLException e) {
            System.err.println(e);
...
public boolean eliminar(ClsObjetos clase) {
        PreparedStatement ps=null;
        Connection con = getConexion();
    ...
    public ClsObjetos devuelvePersona(int idPersona) {
        ClsObjetos persona = new ClsObjetos();
