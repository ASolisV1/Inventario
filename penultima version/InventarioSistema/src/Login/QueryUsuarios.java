...
public class QueryUsuarios extends Conexion {
    ...
    public boolean guardar(ClsUsuarios usu) {
        PreparedStatement ps=null;
        Connection con = getConexion();
        String sql="INSERT INTO usuario (usuarionom,usuariopassword) VALUES (?,?)";
        try {            
            ps = con.prepareStatement(sql);
            ps.setString(1,usu.getUsuarionom());            
            ps.setString(2,usu.getUsuariopassword());            
...
public boolean modificar(ClsUsuarios usu) {
        PreparedStatement ps=null;
        Connection con = getConexion();
        String sql="UPDATE usuario SET usuarionom=?,usuariopassword=? WHERE usuarioid=?";
        try {            
            ps = con.prepareStatement(sql);
            ps.setString(1,usu.getUsuarionom());            
            ps.setString(2,usu.getUsuariopassword());            
            ps.setInt(3,usu.getUsuarioid());            
            ps.execute();
            return true;
        } catch(SQLException e) {
            System.err.println(e);
...
public boolean eliminar(ClsUsuarios usu) {
    ...
    public javax.swing.table.DefaultTableModel listar(String valor) {
        ...
        String sql = "SELECT usuarioid,usuarionom,usuariopassword FROM usuario WHERE usuarionom LIKE ? ORDER BY usuarionom";
        try {
            ...
            while (rs.next()) {
                ...
                registros[2] = rs.getString(3);  // password
